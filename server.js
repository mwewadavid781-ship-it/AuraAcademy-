...
app.post('/api/syllabus/mark-topic', async (req, res) => {
  try {
    ...
    await supabase.from('student_stats').update({
      aura_points: (await supabase.from('student_stats').select('aura_points').eq('student_id', syllabus.student_id).single()).data.aura_points + 10,
      total_topics_completed: (await supabase.from('student_stats').select('total_topics_completed').eq('student_id', syllabus.student_id).single()).data.total_topics_completed + 1
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const simplified = await groq.chat.completions.create({
      ...
      messages: [{ 
        role: 'user', 
        content: `Simplify this study material into 5 KEY POINTS:\n${text}\n\nUse bullet points, simple language. Highlight definitions and important facts.` 
      }],
      max_tokens: 500
...
<textarea id="topics" placeholder="Topics (comma-separated)&#10;e.g. Loops, Functions, Arrays, Databases" style="height:100px"></textarea>
...
<div class="card">
<div style="font-weight:700;color:#4AF2A1;margin-bottom:12px">📄 Upload & Simplify Notes</div>
...
<div style="color:#4AF2A1;font-weight:700;margin-bottom:8px">✅ Simplified Summary:</div>
...
<div style="color:#cbd5e1;font-size:13px;margin-bottom:12px">After your 7-day trial, pay to keep uploading and tracking progress</div>
...
<p class="subtitle">Unlimited uploads, progress tracking, and AI summaries</p>
<div class="card">
...
class App {
  ...
  async handleUpload(e) {
    ...
    if (!sub.isTrialActive) {
      alert('⏰ Trial expired! Pay K10/week to continue uploading.');
      return;
    }
...
app.listen(PORT, () => console.log('🚀 AURA REFINED V1 on ' + PORT));
