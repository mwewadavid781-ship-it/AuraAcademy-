...
app.post('/auth/register', async (req, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from('students').insert([{
      name, email, year, trial_start_date: new Date().toISOString(), trial_end_date: trialEnd.toISOString(),
      is_paid: false, last_login: new Date().toISOString(), study_streak: 1
    }]).select();
    if (error) throw error;
    await supabase.from('student_stats').insert([{ student_id: data[0].id, aura_points: 0, level: 1, total_topics_completed: 0 }]);
...
app.post('/auth/login', async (req, res) => {
  ...
  try {
    ...
    await supabase.from('students').update({ last_login: now.toISOString(), study_streak: newStreak }).eq('id', data.id);
    res.json({ success: true, student: { ...data, study_streak: newStreak }, daysLeft: Math.max(0, daysLeft), isTrialActive });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
});
...
app.post('/api/syllabus/create', async (req, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from('syllabuses').insert([{
      student_id: studentId, course_name: courseName, course_code: courseCode, semester, topics: topicsArray, exam_date: examDate,
      completed_topics: [], progress_percentage: 0, created_at: new Date().toISOString()
    }]).select();
    if (error) throw error;
...
app.post('/api/syllabus/mark-topic', async (req, res) => {
  ...
  try {
    ...
    if (!completed.includes(topicName)) { completed.push(topicName); }
    ...
    const { data, error } = await supabase.from('syllabuses').update({
      completed_topics: completed, progress_percentage: progressPercentage
    ...
    await supabase.from('student_stats').update({ aura_points: newPoints, total_topics_completed: newTopics }).eq('student_id', syllabus.student_id);
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (!isTrialActive) { return res.status(403).json({ error: 'Trial expired' }); }
...
app.post('/api/payment/confirm', async (req, res) => {
  ...
  try {
    ...
    await supabase.from('students').update({
      is_paid: true, last_payment_date: new Date().toISOString(),
  ...
  async loadCourses() {
    ...
    const html = this.courses.map(c => \`
      <div class="course-box">
        <div style="font-weight:700;color:#4AF2A1">\${c.course_name} (\${c.course_code})</div>
