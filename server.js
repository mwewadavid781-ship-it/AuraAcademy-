...
const supabase = createClient(
  process.env.SUPABASE_URL || 'https://fcmbhxwhfxklwxvggech.supabase.co',
  process.env.SUPABASE_KEY || 'sb_publishable_BueIEB3eheoTz85QFm0i5w_gpFvXqoo'
);
...
app.post('/auth/login', async (req, res) => {
  try {
    ...
    const newStreak = daysDiff === 1 ? (data.study_streak || 0) + 1 : 1;
    
    await supabase.from('students')
      .update({ last_login: now.toISOString(), study_streak: newStreak })
      .eq('id', data.id);
...
app.post('/api/syllabus/mark-topic', async (req, res) => {
  try {
    ...
    const statsRes = await supabase
      .from('student_stats')
      .select('aura_points, total_topics_completed')
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    if (!isTrialActive) {
      return res.status(403).json({ error: 'Trial expired. Pay K10/week.' });
    }
    ...
    // Mock AI summary for now (Groq optional)
    const summary = `📋 Study Summary:\n\n✓ Key Point 1: Important concept from your material\n✓ Key Point 2: Critical definition\n✓ Key Point 3: Main formula or principle\n✓ Key Point 4: Practical application\n✓ Key Point 5: Exam tip`;
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', server: 'AURA running' });
});
...
.top-nav{background:rgba(10,10,36,0.8);padding:16px 20px;border-bottom:1.5px solid rgba(74,242,161,0.2);display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:50;backdrop-filter:blur(10px)}
...
.bottom-nav{position:fixed;bottom:0;left:0;right:0;background:rgba(10,10,36,0.8);border-top:1.5px solid rgba(74,242,161,0.2);display:flex;justify-content:space-around;padding:10px 0;z-index:100;backdrop-filter:blur(10px)}
...
.feature-card{background:rgba(74,242,161,0.08);border:1.5px solid rgba(74,242,161,0.2);border-radius:12px;padding:20px}
.feature-icon{font-size:40px;margin-bottom:12px}
.feature-title{font-weight:700;color:#4AF2A1;margin-bottom:8px}
.feature-text{font-size:12px;color:#cbd5e1}
.pricing-box{background:rgba(74,242,161,0.1);border:1.5px solid rgba(74,242,161,0.3);border-radius:12px;padding:30px;max-width:500px;margin-bottom:40px;text-align:center}
.cta-buttons{display:flex;gap:16px;justify-content:center;flex-wrap:wrap;flex-direction:column;max-width:400px}
.auth-section{display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px}
.auth-box{background:rgba(10,10,36,0.7);border:1.5px solid rgba(74,242,161,0.3);border-radius:12px;padding:40px;max-width:420px;width:100%;backdrop-filter:blur(20px)}
.auth-title{font-size:24px;font-weight:800;margin-bottom:8px}
.auth-subtitle{font-size:13px;color:#cbd5e1;margin-bottom:20px}
.toggle-link{text-align:center;margin-top:16px;font-size:13px}
.toggle-link a{color:#4AF2A1;cursor:pointer;text-decoration:none;font-weight:600}
</style></head><body>
...
<div style="font-size:28px;font-weight:800;color:#4AF2A1;margin-bottom:16px">💎 K10/week</div>
<div style="color:#cbd5e1"><strong>7 Days FREE</strong> • Then K10/week • MTN Mobile Money</div>
</div>

<div class="cta-buttons">
<button class="btn" style="width:100%;padding:14px" onclick="app.goTo('login')">SIGN IN</button>
<button class="btn" style="width:100%;padding:14px;margin-top:8px" onclick="app.goTo('signup')">CREATE ACCOUNT</button>
</div>
</div>
</div>
...
<select id="semester" required><option value="">Select Semester</option><option value="Semester 1">Semester 1</option><option value="Semester 2">Semester 2</option></select>
<textarea id="topics" placeholder="Topics (comma-separated)" style="height:100px" required></textarea>
...
<div style="color:#4AF2A1;font-weight:700;margin-bottom:8px">✅ Summary:</div>
<div id="summaryText" style="font-size:13px;color:#cbd5e1;line-height:1.6;white-space:pre-wrap"></div>
</div>
</div>
</div>
...
<div class="card">
<div style="font-weight:700;color:#4AF2A1;margin-bottom:12px">MTN Mobile Money</div>
<input type="tel" id="mtnNumber" placeholder="0964969767" required>
...
class App {
  ...
  async handleLogin(e) {
    ...
    try {
      const res = await fetch(this.API + '/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }) }).then(r => r.json());
      if (res.success) {
  ...
  async handleSignup(e) {
    ...
    try {
      const res = await fetch(this.API + '/auth/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, year }) }).then(r => r.json());
      if (res.success) {
        this.user = res.student;
        localStorage.setItem('aura_user', JSON.stringify(this.user));
        await this.showDashboard();
  ...
  async loadCourses() {
    ...
    const html = this.courses.map(c => \`<div class="course-box"><div style="font-weight:700;color:#4AF2A1">\${c.course_name} (\${c.course_code})</div><div style="font-size:12px;color:#cbd5e1;margin-bottom:8px">\${c.semester}</div><div class="progress-bar"><div class="progress-fill" style="width:\${c.progress_percentage}%"><\/div><\/div><div style="font-size:11px;color:#cbd5e1;margin-top:4px">\${c.progress_percentage}% (\${(c.completed_topics || []).length}/\${c.topics.length})</div><div class="topics-list">\${c.topics.map(t => \`<div class="topic-item \${(c.completed_topics || []).includes(t) ? 'done' : ''}"><span>\${t}<\/span>\${!(c.completed_topics || []).includes(t) ? \`<button class="topic-btn" onclick="app.markTopic(\${c.id},'\${t}')">✓<\/button>\` : '<span style="color:#4AF2A1">✓<\/span>'}<\/div>\`).join('')}<\/div><\/div>\`).join('');
  ...
  async createCourse() {
    ...
    if (!courseName || !courseCode || !semester || !topicsText) { alert('Fill all fields'); return; }
    const topics = topicsText.split(',').map(t => t.trim()).filter(t => t);
    const res = await fetch(this.API + '/api/syllabus/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId: this.user.id, courseName, courseCode, semester, topics, examDate }) }).then(r => r.json());
    if (res.success) { alert('✅ Course created!'); document.getElementById('courseName').value = ''; await this.loadCourses(); this.goTo('dashboard'); }
  }

  async markTopic(syllabusId, topicName) {
    await fetch(this.API + '/api/syllabus/mark-topic', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ syllabusId, topicName }) });
    await this.loadCourses();
    await this.loadStats();
  }
  ...
  async handleUpload(e) {
    ...
    if (!file || this.courses.length === 0) { alert('Create a course first!'); return; }
    const sub = await fetch(this.API + \`/api/subscription/\${this.user.id}\`).then(r => r.json());
    if (!sub.isTrialActive) { alert('Trial expired!'); return; }
  ...
  async initiatePayment() {
    ...
    if (!mtn) { alert('Enter MTN'); return; }
    const res = await fetch(this.API + '/api/payment/initiate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId: this.user.id, msisdn: mtn }) }).then(r => r.json());
    if (res.success) { alert('Check your phone!'); setTimeout(() => this.confirmPayment(), 3000); }
  }
  ...
  goTo(section) {
    ...
    if (event?.target?.closest('.nav-btn')) event.target.closest('.nav-btn').classList.add('active');
  }
...
app.listen(PORT, () => {
  console.log(\`✅ AURA LIVE on port \${PORT}\`);
  console.log(\`📱 Visit: https://auraacademy.onrender.com\`);
});
